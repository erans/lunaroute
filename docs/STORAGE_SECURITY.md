...
**What**: Validates session IDs to prevent directory traversal attacks.

**Implementation**: `session.rs:24-56`
...
- Validated on all SessionStore trait methods
...
**Why**: Prevents arbitrary file access via crafted session IDs.
...
| **Path traversal** | Session ID validation | `session.rs:24-56` |
